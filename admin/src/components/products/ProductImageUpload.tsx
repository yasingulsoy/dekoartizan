...
interface ProductImageUploadProps {
  ...
  onImageReorder?: (imageIds: number[]) => Promise<void>;
}
...
const SortableImage: React.FC<SortableImageProps> = ({
  ...
  return (
    ...
    >
      ...
      <div className="absolute bottom-2 left-2 bg-gray-800/70 text-white text-xs px-2 py-1 rounded flex items-center gap-1">
      ...
      <div className="absolute inset-0 bg-black/60 opacity-0 group-hover:opacity-100 transition-opacity flex items-center justify-center gap-2">
        <label className="cursor-pointer">
          <input
            type="file"
            accept="image/*"
            onChange={(e) => handleFileInputChange(image.id, e)}
            className="hidden"
...
const ProductImageUpload: React.FC<ProductImageUploadProps> = ({
  ...
  const [sortedImages, setSortedImages] = useState<UploadedImage[]>(() => 
    [...uploadedImages].sort((a, b) => (a.display_order || 0) - (b.display_order || 0))
  );
  const [isReordering, setIsReordering] = useState(false);
  const prevUploadedImagesRef = useRef<string>('');

  // uploadedImages değiştiğinde sortedImages'ı güncelle (sadece gerçekten değiştiğinde)
  React.useEffect(() => {
    const currentImagesString = JSON.stringify(uploadedImages.map(img => ({ id: img.id, display_order: img.display_order })));
    
    if (prevUploadedImagesRef.current !== currentImagesString) {
      const sorted = [...uploadedImages].sort((a, b) => (a.display_order || 0) - (b.display_order || 0));
      setSortedImages(sorted);
      prevUploadedImagesRef.current = currentImagesString;
    }
  }, [uploadedImages]);
  ...
  const handleReorder = useCallback(async () => {
    if (!onImageReorder || !productId) return;
    
    const imageIds = sortedImages.map((img) => img.id);
    setIsReordering(true);
    try {
      await onImageReorder(imageIds);
    } catch (error) {
  ...
  }, [sortedImages, onImageReorder, productId, uploadedImages]);
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between">
              <p className="text-sm text-gray-600 dark:text-gray-400">
                Resimleri sürükleyerek sıralayabilirsiniz. İlk resim ana resim olacaktır.
              </p>
              {onImageReorder && productId && (
                <button
