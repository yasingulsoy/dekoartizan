...
const ProductCard = ({ data }: ProductCardProps) => {
  ...
  return (
    <div className="flex items-start space-x-4">
      <Link
        href={`/magaza/urunler/${data.slug || data.name.toLowerCase().split(" ").join("-")}`}
      ...
      <div className="flex w-full self-stretch flex-col">
        ...
        <div className="-mt-1">
          <span className="text-black text-xs md:text-sm mr-1">Beden:</span>
          <span className="text-black/60 text-xs md:text-sm">
            {data.attributes[0]}
          </span>
        </div>
        <div className="mb-auto -mt-1.5">
          <span className="text-black text-xs md:text-sm mr-1">Renk:</span>
          <span className="text-black/60 text-xs md:text-sm">
            {data.attributes[1]}
          </span>
        </div>
